...
def generate_barcode_lls(barcode_pos_reads, genotypes, donors, num_donors,
    ...
    for base in 'ACGT':
        ...
        pos_P_donor_given_ref = P_donor_given_ref.loc[positions]
        pos_P_donor_given_alt = P_donor_given_alt.loc[positions]
        temp_probs += pos_P_donor_given_ref * base_is_ref * barcode_pos_reads[f'prob_{base}'].values.reshape((-1, 1))
        temp_probs += pos_P_donor_given_alt * base_is_alt * barcode_pos_reads[f'prob_{base}'].values.reshape((-1, 1))
...
def dropulation_likelihoods(barcode_reads, genotypes, donor_ref_cts, donor_alt_cts, donors, error_rate=0.001, het_rate=0.5):
    ...
    donor_genotypes = replaced_alt[replaced_ref == replaced_alt].fillna('none') # includes counting hets?
...
def main():
    ...
    with gzip.open(f"barcode_log_likelihood_{simplified_region_name}.{args.likelihood_method}.unoptimized.txt.gz", "wb") as outf:
